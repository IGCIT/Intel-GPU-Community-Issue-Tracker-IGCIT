...
IGCITDriverSwitch::IGCITDriverSwitch(QWidget *parent): QMainWindow(parent) {
    ui.setupUi(this);
    ...
    QObject::connect(menuDwnlSrc, &QAction::triggered, this, &IGCITDriverSwitch::onMenuDwnlSrcTriggered);
    QObject::connect(menuDwnlStableArcXe, &QAction::triggered, this, &IGCITDriverSwitch::onMenuDwnlStableArcXeTriggered);
    QObject::connect(menuDwnlBetaArcXe, &QAction::triggered, this, &IGCITDriverSwitch::onMenuDwnlBetaArcXeTriggered);
    QObject::connect(menuDwnlStable7_10, &QAction::triggered, this, &IGCITDriverSwitch::onMenuDwnlStable7_10Triggered);
    QObject::connect(menuHelpWiki, &QAction::triggered, this, &IGCITDriverSwitch::onMenuHelpWikiTriggered);
    QObject::connect(menuHelpAbout, &QAction::triggered, this, &IGCITDriverSwitch::onMenuHelpAboutTriggered);
    QObject::connect(ui.driversListCombo, &QComboBox::currentIndexChanged, this, &IGCITDriverSwitch::onDriverComboIndexChanged);
    QObject::connect(ui.restoreBtn, &QPushButton::clicked, this, &IGCITDriverSwitch::onRestoreBtnClicked);
    QObject::connect(ui.loadBtn, &QPushButton::clicked, this, &IGCITDriverSwitch::onLoadBtnClicked);
    ...
    if (!QDir(driverDirPath).exists() && !QDir().mkdir(driverDirPath)) {
        ...
        QObject::connect(fsWatcher.get(), &QFileSystemWatcher::directoryChanged, this, &IGCITDriverSwitch::onDirectoryChanged);
    }

    unlockUI();
}

QString IGCITDriverSwitch::getCurrentDriverVersion() const {
    ...
    ret = RegOpenKeyExA(HKEY_LOCAL_MACHINE, "HARDWARE\\DEVICEMAP\\VIDEO", 0, KEY_READ, &rkey);
    ...
    for (DWORD i=0; i<valuesCount; ++i) {
        ...
        valueStr.erase(0, strlen("\\Registry\\Machine\\")); // should be the same for all, so no parse but remove
...
QList<QString> IGCITDriverSwitch::getDeviceIDs() const {
...
void IGCITDriverSwitch::runGetDriversListThread() {
    ui.driversListCombo->setEnabled(false);

    if (!getDriversListThd.isNull() && getDriversListThd->isRunning()) {
        QObject::disconnect(getDriversListThd, &GetDriversListThread::resultReady, this, &IGCITDriverSwitch::onGetDriversListThdResultReady);
        QObject::disconnect(getDriversListThd, &GetDriversListThread::logMessageWritten, this, &IGCITDriverSwitch::writeLog);
    ...
    QObject::connect(getDriversListThd, &GetDriversListThread::resultReady, this, &IGCITDriverSwitch::onGetDriversListThdResultReady);
    QObject::connect(getDriversListThd, &GetDriversListThread::logMessageWritten, this, &IGCITDriverSwitch::writeLog);
...
void IGCITDriverSwitch::runExtractDriverForInstallThread(const QString &fileName) {
    extractDriverForinstallThd = new ExtractDriverForInstallthread(this);

    extractDriverForinstallThd->setPaths(QString("%1/%2").arg(driverDirPath).arg(fileName), tmpExtractDirPath);
    ...
    QObject::connect(extractDriverForinstallThd, &ExtractDriverForInstallthread::logMessageWritten, this, &IGCITDriverSwitch::writeLog);
    QObject::connect(extractDriverForinstallThd, &ExtractDriverForInstallthread::progressUpdated, this, &IGCITDriverSwitch::onExtractDriverForInstallThdProgressUpdated);
    QObject::connect(extractDriverForinstallThd, &ExtractDriverForInstallthread::resultReady, this, &IGCITDriverSwitch::onExtractDriverForInstallThdResultReady);

    extractDriverForinstallThd->start();
}

std::wstring IGCITDriverSwitch::getInstallerArgStr() const {
...
void IGCITDriverSwitch::onMenuDwnlBetaArcXeTriggered() {
...
void IGCITDriverSwitch::onMenuDwnlStable7_10Triggered() {
...
void IGCITDriverSwitch::onGetDriversListThdResultReady(const QList<QString> &driverList, const QList<QString> &versionList) {
    QSignalBlocker sblock {ui.driversListCombo};
...
void IGCITDriverSwitch::onRestoreBtnClicked() {
    ...
    if (ShExecInfo.hProcess != 0) {
        ...
        QObject::connect(restoreDriverWaitThd, &RestoreDriverWaitThread::resultReady, this, &IGCITDriverSwitch::onRestoreDriverWaitThdResultReady);
...
void IGCITDriverSwitch::onDriverComboIndexChanged(int idx) {
...
void IGCITDriverSwitch::onLoadBtnClicked() {
    QString selected = ui.driversListCombo->currentText();
    QString fileName;
    int selectedIdx;
    ...
    writeLog(QString("Selected: %1 (%2)").arg(selected).arg(fileName), Qt::darkBlue);
    runExtractDriverForInstallThread(fileName);
}

void IGCITDriverSwitch::onInstallerProcWaitResultReady(DWORD code) {
...
void IGCITDriverSwitch::onRestoreDriverWaitThdResultReady() {
    QString driverVer = getCurrentDriverVersion();
...
void IGCITDriverSwitch::onExtractDriverForInstallThdProgressUpdated(int progress) {
    ui.progressBar->setValue(progress);
}

void IGCITDriverSwitch::onExtractDriverForInstallThdResultReady(bool res) {
    SHELLEXECUTEINFO ShExecInfo {};
    ...
    if (ShExecInfo.hProcess != 0) {
        ...
        QObject::connect(installerProcWaitThd, &InstallerProcessWaitThread::resultReady, this, &IGCITDriverSwitch::onInstallerProcWaitResultReady);
